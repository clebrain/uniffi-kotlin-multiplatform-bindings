...
internal object FfiConverterDuration : FfiConverterRustBuffer<kotlin.time.Duration> {
    override fun read(buf: NoCopySource): kotlin.time.Duration {
        ...
        if (duration < 0.nanoseconds) {
            throw kotlin.IllegalArgumentException("Duration nanoseconds exceed minimum or maximum supported by uniffi")
        }
        return duration
    }
    ...
    override fun write(value: kotlin.time.Duration, buf: okio.Buffer) {
        if (value < 0.nanoseconds) {
            throw kotlin.IllegalArgumentException("Invalid duration, must be non-negative")
        }
