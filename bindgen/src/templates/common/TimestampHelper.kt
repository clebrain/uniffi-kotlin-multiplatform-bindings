internal object FfiConverterTimestamp : FfiConverterRustBuffer<kotlinx.datetime.Instant> {
    override fun read(buf: NoCopySource): kotlinx.datetime.Instant {
        ...
        if (nanoseconds < 0) {
            throw kotlin.IllegalArgumentException("Instant nanoseconds exceed minimum or maximum supported by uniffi")
        }
        return instant
    }
    ...
    override fun write(value: kotlinx.datetime.Instant, buf: okio.Buffer) {
        if (value.nanosecondsOfSecond < 0) {
            throw kotlin.IllegalArgumentException("Invalid timestamp, nano value must be non-negative")
        }
